...
def generate_2d_concatenated_self_attention_relative_positional_encoding_index(z_shape, x_shape):
    ...
    concatenated_2d_index_h = torch.cat((z_2d_index_h, x_2d_index_h, x_2d_index_h))
    concatenated_2d_index_w = torch.cat((z_2d_index_w, x_2d_index_w, x_2d_index_w))
    ...
    a = torch.empty((z_len + x_len + x_len), dtype=torch.int64)
    a[:z_len] = 0
    a[z_len:z_len + x_len] = 1
    a[z_len + x_len:] = 2
    b=a[:, None].repeat(1, z_len + x_len + x_len)
    c=a[None, :].repeat(z_len + x_len + x_len, 1)

    diff = torch.stack((diff_h, diff_w, b, c), dim=-1)
    _, indices = torch.unique(diff.view((z_len + x_len + x_len) * (z_len + x_len + x_len), 4), return_inverse=True, dim=0)
    return indices.view((z_len + x_len + x_len), (z_len + x_len + x_len))
...
def generate_2d_concatenated_cross_attention_relative_positional_encoding_index(z_shape, x_shape):
    ...
    _, indices = torch.unique(diff.view(x_len * (z_len + x_len + x_len), 3), return_inverse=True, dim=0)
    return indices.view(x_len, (z_len + x_len + x_len))
