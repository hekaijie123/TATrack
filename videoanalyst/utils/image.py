...
##############################################################test
    img = torchvision.io.image.read_image(img_file, torchvision.io.image.ImageReadMode.RGB)

    # x_ch0 = (torch.unsqueeze(img[:, 2], 1) - mean[2]) / std[2]
    # x_ch1 = (torch.unsqueeze(img[:, 1], 1) - mean[1]) / std[1]
    # x_ch2 = (torch.unsqueeze(img[:, 0], 1) - mean[0]) / std[0]
