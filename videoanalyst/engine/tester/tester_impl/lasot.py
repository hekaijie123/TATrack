...
class LaSOTTester(TesterBase):
    ...
    def update_params(self):
        ...
        if num_gpu > 0:
            all_devs = [torch.device("cuda:%d" % 0) for i in range(num_gpu)]
            # all_devs = [torch.device("cuda:0"),torch.device("cuda:0"),torch.device("cuda:0"),torch.device("cuda:0"),torch.device("cuda:0"),torch.device("cuda:0"),torch.device("cuda:1"),]
        else:
