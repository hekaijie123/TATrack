...
class TATrackTracker(PipelineBase):
    ...
    def select_representatives(self, cur_frame_idx):        ##用某种方式更新队列
        ...
        for i in range(cur_frame_idx-1,-1,-1):
            if(indexes == None):
                if(self._state['pscores'][i]>=scores.mean() ):#and scores[i] >scores[i-1]
                    indexes = i
            else:break

        # sorce = self._state['penalty_scores'].mean()
        # indexes = None
        # for i in range(cur_frame_idx-1,-1,-1):
        #     if(indexes == None):
        #         if(self._state['pscores'][i]>=sorce):  #  and self._state['qscores'][i]>=self._state['qscores'].mean()
        #             indexes = i
        #     else:break

        # print("idx",indexes,"curent_sorce",self._state['pscores'][cur_frame_idx-1],"mean",self._state['pscores'].mean())    #,"penalty_mean",scores
        # if (indexes == None):
        # _,indexes = self._state['pscores'].topk(1)
    ...
    def init(self, im, bbox):
        ...
        self._state['im_w'] = im.shape[2]       ##检查
        ...
        self._state['last_bbox'] = bbox     ##初始，xywh
        self._state["rng"] = np.random.RandomState(123456)
        self._state['avg_chans'] = get_image_mean(im)
        self.window = torch.flatten(torch.outer(torch.hann_window(14, periodic=False),
                                                    torch.hann_window(14, periodic=False)))
        self.window = self.window.to(self.device)
        last_bbox = xywh2xyxy(self._state['last_bbox']) 
        curation_parameter, _ = prepare_SiamFC_curation(last_bbox, self._hyper_params['template_area_factor'], [self._hp_m_size,self._hp_m_size])##缩放参数和中心位置
        curated_first_frame_image, _ = do_SiamFC_curation(im, [self._hp_m_size,self._hp_m_size], curation_parameter, self._hyper_params['interpolate_mode'])##裁剪后图片和颜色平均值
        curated_first_frame_image = self.transform(curated_first_frame_image)   ##imagenet的归一化，没有任何其他增强
        # with torch.no_grad():
        #     data = curated_first_frame_image.unsqueeze(0).to(self.device)
        #     self._state['template_feature'] = self._model.basemodel_m(data,reshape=False) 
        ...
        #                         last_bbox,self._state['avg_chans'])      ###尝试几十帧后更新平均通道
        # self._state['prev_frame_feat'] = prev_frame_feat
        # self._state['prev_gauss_label'] = prev_gauss_label
        # self._state['prev_ltbr_label'] = prev_ltbr_label
    ...
    def update(self, im, state=None):
        ...
        if state is None:
            last_bbox = xywh2xyxy(self._state['last_bbox'])     ##先转换为xyxy

        fidx = self._state['cur_frame_idx']
        # scores = self._state['pscores']
        # if (scores[fidx-1]>=scores[1:].mean()):
        # if self._state['pscores'][-1]>= 0.8:
        prev_frame_feat,prev_gauss_label,prev_ltbr_label = self.memorize(self._state['last_img'],
                                last_bbox,self._state['avg_chans'])      ###尝试几十帧后更新平均通道
        ...
        output = self.track(im,
                            last_bbox,
                            features,gauss_label,ltbr_label,
                            # self._state['prev_frame_feat'],self._state['prev_gauss_label'],self._state['prev_ltbr_label'],
    ...
    def memorize(self, im: np.array, last_bbox, avg_chans):
        ...
        # last_bbox = _adjust_bbox_size(torch.tensor(last_bbox),[self._hyper_params['min_w'],self._hyper_params['min_h']])
        curation_parameter, output_bbox = prepare_SiamFC_curation(last_bbox, 4.0 , [q_size,q_size])##缩放参数和中心位置
        curated_first_frame_image, _ = do_SiamFC_curation(im, [q_size,q_size], curation_parameter, self._hyper_params['interpolate_mode'])##裁剪后图片和颜色平均值
        curated_first_frame_image = self.transform(curated_first_frame_image)   ##imagenet的归一化，没有任何其他增强
        
        gauss_label = gaussian_label_function(output_bbox.view(1,-1),0.1,1,14, q_size, end_pad_if_even=True).view(1,-1,1)
        ...
        # with torch.no_grad():
        #     data = curated_first_frame_image.unsqueeze(0).to(self.device)
        #     fm = self._model(data, ltbr_label, gauss_label, phase=phase)
    ...
    def post_tracking(self,outputs):
        ...
        bbox = xyxy2xywh(bbox)
        # bbox = clip_box(bbox,self._state['im_h'],self._state['im_w'],margin=15)
