...
class COCODataset(DatasetBase):
    ...
    def update_params(self):
        ...
        if len(COCODataset.data_items) == 0:
            self._ensure_cache()            ##自定义的参数是否已经更新？
    ...
    def _ensure_cache(self):
        ...
        for subset in subsets:
            ...
            if self._hyper_params["with_mask"]:
                ...
                nfs_suffix = ''
                if dataset_root.find('/data/compute03') >= 0 and not os.path.exists('/mnt/ssddata'):
                    nfs_suffix = '_data'
                elif dataset_root.find('nfs_') >= 0 and not os.path.exists('/mnt/ssddata'):
                    nfs_suffix = '_nfs'
                else:
                    pass
                cache_file = osp.join(dataset_root,
                                      "cache/coco_bbox_{}.pkl".format(subset + nfs_suffix))
            if osp.exists(cache_file):
                with open(cache_file, 'rb') as f:
                    COCODataset.data_items += pickle.load(f)        ##内容的型式是什么样？
                ...
                with contextlib.redirect_stdout(io.StringIO()):
                    ...
                    img_ids = sorted(coco_api.imgs.keys())      ##排序后的结果是什么样？
                ...
                for (img_dict, anno_dict_list) in imgs_anns:        ###处理前imgs_anns的型式？
                    record = {}
