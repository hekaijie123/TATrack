...
class TATrack(ModuleBase):
    ...
    def __init__(self, backbone, neck, head, loss=None):   
        ...
        # self.basemodel_q = backbone      ########只用一个主干网络
        self.neck = neck
        self.head = head
        self.loss = loss
        self._phase = "train"
    ...
    def memorize(self, im_crop, ltbr_label, gauss_label):
        ...
        # if self.neck.z_input_projection is not None:
        #     fm = self.z_input_projection(fm)
        # ltbr_label = ltbr_label.to(device = im_crop.device,dtype=torch.float)
        # gauss_label = gauss_label.to(device = im_crop.device,dtype=torch.float)
        # fp += self.neck.box_encoding(ltbr_label,gauss_label)
        # fm = self.neck_m(fm)
        # fm = fm.permute(1, 0, 2, 3).unsqueeze(0).contiguous()  # B, C, T, H, W
        # fm = fm.view(*fm.shape[:2],-1).transpose(1,2)
        return fp
    ...
    def train_forward(self, training_data):
        ...
        # fm = self.basemodel_m(memory_img,False)           ##B*T,384,14,14     , target_fg_bg_label_map
        # if self.neck.z_input_projection is not None:
        #     fm = self.z_input_projection(fm)
        # fp = self.basemodel_q(pre_img,False)   
        fp += self.neck.box_encoding(training_data["ltbr_label"],training_data["gauss_label"])
        # fq = self.basemodel_q(query_img,False)
        # if self.neck.x_input_projection is not None:
        #     fq = self.x_input_projection(fq)
    ...
    def forward(self, *args, phase=None):
        ...
        if phase == 'train':
            ...
            # fq = self.basemodel_q(search_img,False)
            # B, C, H, W = fq.size()
            # if self.neck.x_input_projection is not None:
            #     fq = self.x_input_projection(fq)
            ...
            fp += self.neck.box_encoding(ltbr_label,gauss_label)
            # fm = self.neck.memory_fuse(fm,fq)
            # fq = fq.view(B,C,H*W).transpose(1,2)
