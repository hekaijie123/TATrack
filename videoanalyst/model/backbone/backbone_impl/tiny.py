...
class tiny(ModuleBase):
    ...
    def update_params(self):
        ...
        #         drop=0.0,
        #         attn_drop=0.0,
        #         drop_path=self.backbone.stages[2].drop_path[i] if isinstance(self.backbone.stages[2].drop_path, list) else self.backbone.stages[2].drop_path,
        #         norm_layer=nn.LayerNorm)
        #     for i in range(6)])
        # weight = self.backbone.stages[2].blocks.state_dict()

        # prefix=''
        # for full_key in list(weight.keys()):
        #     key: str = full_key[len(prefix):]
        #     if key.startswith('blocks.'):
        #         weight[full_key.replace('blocks.', prefix)] = weight.pop(full_key)
            # if key.startswith('pre_stage.'):
            #     weight.pop(full_key)
        # self.blocks.load_state_dict(weight)
        self.window_size = 7
        ...
        with drop_path_allocator:
            self.encoder = build_encoder(self.config, drop_path_allocator,
                                        384, 8, 4.0, True, 0.0, 0.0,[7,7], [14,14])
    ...
    def forward(self, z, pre_z, x, reshape=True):
        ...
        for i,blk in enumerate(self.backbone.stages[2].blocks):
            ...
            if(i == 1):
                z,pre_z,x = self.encoder_attention(z,pre_z,x,None,None,0)
                # pre_b = pre_z.mean(dim=-1).reshape(14,14).cpu()
                # x_b = x.mean(dim=-1).reshape(14,14).cpu()
                # fig = plt.figure(constrained_layout=False, figsize=(5, 5), dpi=224)
                # fig.subplots_adjust(left=0.0, bottom=0.0, right=1.0, top=1.0)
                # ax = fig.add_subplot(111)
                # ax.imshow(pre_b, cmap='summer', interpolation='nearest')
                # ax.axis('off')
                # plt.savefig("/home/hkj/code/tiny-visualization/backbone1/pre.png")
                # plt.close()
                # fig = plt.figure(constrained_layout=False, figsize=(5, 5), dpi=224)
                # fig.subplots_adjust(left=0.0, bottom=0.0, right=1.0, top=1.0)
                # ax = fig.add_subplot(111)
                # ax.imshow(x_b, cmap='summer', interpolation='nearest')
                # ax.axis('off')
                # plt.savefig("/home/hkj/code/tiny-visualization/backbone1/x.png")
                # plt.close()

            if(i == 3):
                z,pre_z,x = self.encoder_attention(z,pre_z,x,None,None,1)
                # pre_b = pre_z.mean(dim=-1).reshape(14,14).cpu()
                # x_b = x.mean(dim=-1).reshape(14,14).cpu()
                # fig = plt.figure(constrained_layout=False, figsize=(5, 5), dpi=224)
                # fig.subplots_adjust(left=0.0, bottom=0.0, right=1.0, top=1.0)
                ...
                # ax.imshow(pre_b, cmap='summer', interpolation='nearest')
                # ax.axis('off')
                # plt.savefig("/home/hkj/code/tiny-visualization/backbone2/pre.png")
                # plt.close()
                # fig = plt.figure(constrained_layout=False, figsize=(5, 5), dpi=224)
                # fig.subplots_adjust(left=0.0, bottom=0.0, right=1.0, top=1.0)
                # ax = fig.add_subplot(111)
                # ax.imshow(x_b, cmap='summer', interpolation='nearest')
                # ax.axis('off')
                # plt.savefig("/home/hkj/code/tiny-visualization/backbone2/x.png")
                # plt.close()
