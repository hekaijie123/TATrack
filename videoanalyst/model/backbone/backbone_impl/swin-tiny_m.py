...
def _freeze_stages(self, frozen_stages, ape):
    patch_embed = self.pre_stage
    if frozen_stages >= 0:
...
def _update_state_dict_(state_dict, prefix=''):
    for full_key in list(state_dict.keys()):
        ...
        if key.startswith('layers.'):
            ...
            state_dict[full_key.replace('patch_embed', 'pre_stage')] = state_dict.pop(full_key)
...
class MaskEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=1, embed_dim=96, norm_layer=None, pretrain_img_size=224,
                 ape=False, drop_rate=0.):
        super().__init__()
        patch_size = to_2tuple(patch_size)
        self.patch_size = patch_size
        ...
        self.proj_fg_bg_label_map = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size, padding=0, bias=False)
    ...
    def forward(self, x, H, W):
        ...
        x = self.proj_fg_bg_label_map(x)  # B C Wh Ww
...
class SwinTransformerTM(ModuleBase):
    ...
    def __init__(self,
                 ...
                 frozen_stages=-1,
                 use_checkpoint=False):
        super(SwinTransformerTM, self).__init__()
        ...
        self.ape = ape
        _freeze_stages(self, self.frozen_stages, self.ape)
        self.maskembed = MaskEmbed(
    ...
    def forward(self, x, mask_index, out_indices=None, reshape=True):
        ...
        # mask_index = self.maskembed(mask_index,H,W)
        x, H, W = self.pre_stage(x,H,W)         ##/6,3136,96
        ...
        for i in range(self.num_stages):
            ...
            if i in out_indices:
                ...
                outs.append(x_out)
        return outs[0]      ## if none reshape ,out.shape = B,H*W,C

    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(SwinTransformerTM, self).train(mode)
        # _freeze_stages(self, self.frozen_stages, self.ape)



    def update_params(self):
        self.init_weights(pretrained = 'https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_tiny_patch4_window7_224.pth')
